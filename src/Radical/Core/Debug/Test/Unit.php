...
class Unit {
	...
	public function run($method) {
		...
		$h = function($code, $message, $file, $line = 0, $context = array()) use ($self) {
			...
			$self->_reportException(compact('code', 'message', 'file', 'line', 'trace', 'context'));
		};
		set_error_handler($h);
		
		if ($this->_runTestMethod($method) === false) {
			break;
		}
	...
	public function assert($expression, $message = false, $data = array()) {
		...
		if (strpos($message, "{:message}") !== false) {
			...
			$message = String\Format::sprintfn($message, $params);
		}
	 ...
	 * @filter
	 */
	protected function _runTestMethod($method) {
		try {
			$this->setUp();
		} catch (Exception $e) {
	 ...
	 * @return void
	 */
	public function _handleException($exception, $lineFlag = null) {
		$data = $exception;
	 ...
	 */
	public function _reportException($exception, $lineFlag = null) {
		$message = $exception['message'];

		$isExpected = (($exp = end($this->_expected)) && ($exp === true || $exp == $message || (
			true //Validator::isRegex($exp) && preg_match($exp, $message)
		)));
		if ($isExpected) {
